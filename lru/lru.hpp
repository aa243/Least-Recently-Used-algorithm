...
namespace sjtu
{
	...
	public:
		...
		public:
			...
			iterator(const list_node *p1, const double_list *p2) : p(p1), plist(p2){};
			~iterator(){};
			/**
			 * iter++
			 */
			iterator operator++(int)
			{
				if (plist != nullptr)
				{
					...
					throw invalid_iterator("Iterator is not bounded to a double_list.");
			}
			/**
			 * ++iter
			 */
			...
			iterator operator--(int)
			{
				...
				{
					if (p == plist->pbegin)
						p = nullptr, throw invalid_iterator("begin()--");
					iterator temp = *this;
					if (p != nullptr)
						p = p->pre;
					else
			...
			T &operator*() const
			{
				if (plist == nullptr || p == nullptr)
					throw invalid_iterator("Iterator point to null but is asked to dereference.");
				return *p->pval;
			}
			/**
			 * other operation
